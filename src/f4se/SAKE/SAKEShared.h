...
110 */
class TempAimModel : public BaseFormComponent
{
public:
	virtual			~TempAimModel();
...
/* Keys 
- They were based on TESObjectMISC in skyrim, which have had some additions since then. 
- It seems to work, so maybe it's the same here?
- MiscItems differences: componentss, featured item msg
100 */
...
namespace SAKEData
{
	...
	void LoadOverrides_Ammo(TESAmmo * ammoForm, json & ammoOverride);
